# EMQ-Inspired Design System

## 1. Design Direction

This system is inspired by the visual language observed on [emq.com](https://www.emq.com/): enterprise fintech, API-first, confident, and infrastructure-grade.

Core impression:
- Clean white canvas with dark-indigo headlines
- Strong electric blue as primary action color
- Compact, highly legible typography for business users
- Card-based layout with restrained border radius
- Technical polish over decorative flourish

The style should feel:
- **Trustworthy** (regulated financial infrastructure)
- **Scalable** (B2B platform mindset)
- **Modern** (API and digital economy focus)
- **Efficient** (clear hierarchy, high information density)
...
### Brand Core
- `--emq-primary`: `#4357ef` (primary actions, links)
- `--emq-primary-strong`: `#672bff` (highlight variant, gradient start)
- `--emq-ink-strong`: `#07072d` (headline and key text)
- `--emq-ink`: `#232937` (body text)
- `--emq-surface`: `#ffffff` (base background)
- `--emq-muted-surface`: `#f6f9fc` (section and neutral surfaces)

### Support
- `--emq-danger`: `#ff5c50`
- `--emq-border`: `#e5edf5`
- `--emq-border-strong`: `#c9d2ff`
- `--emq-text-muted`: `#64748d`

### Roles
- Primary CTA background: `#4357ef`
- Primary CTA hover: `#3b4ee0`
- Heading text: `#07072d`
- Body text: `#232937`
- Secondary text: `#64748d`
- Section background alt: `#f6f9fc`

---

## 3. Typography

Observed style on EMQ stack leans toward:
- **Condensed display** for headings
- **Highly readable sans** for body and UI

### Font Pairing
- Heading/UI emphasis: `Barlow Semi Condensed`
- Body/content: `Open Sans`
- Monospace: `Source Code Pro`, `SFMono-Regular`, fallback monospace

### Type Scale

| Role | Size | Weight | Line-height | Letter-spacing | Font |
|---|---:|---:|---:|---:|---|
| Hero Display | 52-56px | 600 | 1.05 | -0.02em | Barlow Semi Condensed |
| Section Heading | 36-42px | 600 | 1.10 | -0.01em | Barlow Semi Condensed |
| Card Heading | 24-30px | 600 | 1.15 | -0.01em | Barlow Semi Condensed |
| Subheading | 18-22px | 600 | 1.20 | normal | Barlow Semi Condensed |
| Body Large | 18px | 400 | 1.45 | normal | Open Sans |
| Body | 16px | 400 | 1.40 | normal | Open Sans |
| Caption | 13-14px | 400 | 1.35 | normal | Open Sans |
| Button Label | 14-16px | 500 | 1.00 | normal | Open Sans |

Typography rules:
- Do not use pure black for headline text.
- Keep headings compact and dark-indigo.
- Keep paragraph text neutral and readable.
...
### Spacing
- Base unit: `8px`
- Recommended rhythm: `8 / 12 / 16 / 24 / 32 / 48 / 64`
- Vertical sections: `py-16` to `py-24` desktop

### Radius
- Primary radius: `4px`
- Secondary radius: `6px`
- Featured card max: `8px`

### Layout
- Max content width: `1080px - 1200px`
- Grid preference: `2` and `3` columns on desktop
- Mobile first: stack with clear block separation
...
### Border
- Default: `1px solid #e5edf5`
- Active/Focus: `1px solid #4357ef` or ring

### Shadow
- Main shadow:
  - `rgba(7, 7, 45, 0.14) 0 15px 35px 0`
  - `rgba(0, 0, 0, 0.08) 0 8px 24px -8px`

Use shadow only on elevated surfaces (dropdowns, cards, sticky headers).
...
- Hover BG: `rgba(67,87,239,0.06)`

### Cards
- BG: `#ffffff`
- Border: `1px solid #e5edf5`
- Radius: `6px`
- Shadow: main shadow token

### Header/Nav
- Surface: white with slight opacity + blur
- Link default: `#07072d`
- Link hover/active: `#4357ef`
- Dropdown panel: white, subtle border, shadow
...
- Easing: `ease` or `cubic-bezier(0.25, 1, 0.33, 1)`
- Hover change should focus on:
  - color shift
  - subtle shadow increase
  - avoid heavy transforms on data-dense UI
...
### Do
- Use `#4357ef` for primary interactions
- Keep headings in `#07072d`
- Keep radius conservative (4-8px)
- Use condensed heading + readable body font pair
- Keep section spacing consistent and grid-aligned

### Don't
- Do not use oversaturated multi-brand gradients for core UI
- Do not use oversized rounded pills as default button shape
- Do not use very light gray text for body copy
- Do not mix too many accent colors in one viewport

---

## 9. Implementation Checklist

1. Global tokens in `styles/globals.css`
2. Fonts wired in root layout (`Open Sans`, `Barlow Semi Condensed`)
3. Base components aligned:
   - Button
   - Card
   - Header/Nav
4. Replace hard-coded legacy blues/purples page by page
5. Validate contrast and responsive behavior

---

## 10. Notes

- This file is intentionally implementation-oriented for engineering use.
- If brand team provides official EMQ token files later, replace hex values with canonical tokens and update this document accordingly.
